...
const updateSoundButton = (): void => {
  soundButton.textContent = sounds.isMuted ? 'SOUND OFF' : 'SOUND ON'
  soundButton.setAttribute('aria-pressed', String(sounds.isMuted))
}
updateSoundButton()
...
const refreshRoomPanel = (snapshot: GameSnapshot): void => {
  if (host?.info.mode !== 'online') return
...
element<HTMLButtonElement>('local-button').addEventListener('click', async () => {
  ...
  try {
    enterGame(await LocalGameHost.create())
  } catch (error) {
...
element<HTMLButtonElement>('host-button').addEventListener('click', async () => {
  ...
  try {
    enterGame(await OnlineGameHost.createRoom())
  } catch (error) {
    message.textContent = `Could not reach the server: ${String(error)}`
    clearBusy()
  }
})
...
element<HTMLButtonElement>('join-button').addEventListener('click', async () => {
  ...
  if (code.length !== 4) {
    message.textContent = 'Enter a four-letter room code.'
    return
  }
  setBusy('Joining room…')
  try {
    enterGame(await OnlineGameHost.joinRoom(code))
  } catch (error) {
...
const updateHud = (snapshot: GameSnapshot): void => {
  ...
  scoreboard.textContent = players
    .map(
      (player) =>
        `P${String((player.playerIndex ?? 0) + 1)}  SCORE ${String(snapshot.round.scores[player.id] ?? 0)}`
    )
    .join('   ·   ')
...
const frame = (now: number): void => {
  ...
  if (host) {
    ...
    activeHost.info.controlledPlayerIds.forEach((playerId, slot) => {
      activeHost.submitInput(playerId, inputs.sample(slot, playerId, before))
    })
    host.update(dt)
    const snapshot = host.getSnapshot()
    sounds.update(snapshot)
