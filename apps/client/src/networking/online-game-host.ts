...
const serverUrl = (): string => {
  ...
  if (import.meta.env.DEV) return `${location.protocol}//${location.hostname}:2567`;
...
export class OnlineGameHost implements GameHost {
  ...
  static async joinRoom(code: string): Promise<OnlineGameHost> {
    const response = await fetch(`${serverUrl()}/api/rooms/${encodeURIComponent(code.toUpperCase())}`);
    if (!response.ok) throw new Error("Room not found. Check the four-letter code.");
  ...
  getSnapshot(): GameSnapshot {
    if (this.previous.tick === 0 || this.latest.tick === this.previous.tick) return this.latest;
    const alpha = Math.min(1, (performance.now() - this.receivedAt) / 50);
    const oldEntities = new Map(this.previous.entities.map((entity) => [entity.id, entity]));
    return {
      ...this.latest,
