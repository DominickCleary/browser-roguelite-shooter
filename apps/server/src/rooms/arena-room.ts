...
export class ArenaRoom extends Room {
  ...
  override async onCreate(): Promise<void> {
    ...
    this.onMessage("input", (client, message: ClientInputMessage) => {
      if (
        !this.playersBySession.get(client.sessionId)?.includes(message.playerId)
      )
        return;
    ...
    this.setSimulationInterval(
      (deltaMilliseconds) => this.simulate(deltaMilliseconds / 1_000),
      1_000 / 60,
    );
  }
  ...
  override onJoin(client: Client): void {
    ...
    const playerId = this.simulation.addPlayer(
      client.sessionId,
      this.playersBySession.size,
    );
  ...
  override onLeave(client: Client): void {
    for (const playerId of this.playersBySession.get(client.sessionId) ?? [])
